...
dmso.data <- Read10X(data.dir = "../data/DMSO_24hr_expt3/")

## Initializing the Seurat object with the raw (non-normalized data).
dmso <- CreateSeuratObject(counts = dmso.data, project = "DMSO24hExp3_PROGENy", min.cells = 3, min.features = 200)
...
dmso <- subset(dmso, subset = nFeature_RNA > 20 & nFeature_RNA < 8000 & percent.mt < 20)
...
write.csv(progeny_scores_df, '../results/pw_activity_dmso_24h_expt3.csv')
