...
trametinib.data <- Read10X(data.dir = "../data/Trametinib_24hr_expt3/")

## Initializing Seurat object with the raw (non-normalized data).
trametinib <- CreateSeuratObject(counts = trametinib.data, project = "Trametinib24hExp3", min.cells = 3, min.features = 200)
...
trametinib <- subset(trametinib, subset = nFeature_RNA > 20 & nFeature_RNA < 6000 & percent.mt < 20)
...
write.csv(progeny_scores_df, '../results/pw_activity_trametinib_24h_expt3.csv')
